...
> Deploy to Render.com (free): Push this repo to GitHub → New Web Service → `uvicorn backend.main:app --host 0.0.0.0 --port $PORT`
...
3. **Retrieves** the most applicable NIST SP 800-53 Rev. 5 control per risk via semantic RAG (ChromaDB + sentence-transformers)
...
5. **Renders** a premium dark-mode dashboard — readable by a technical manager without further processing
...
```bash
git clone https://github.com/YOUR_USERNAME/tawasol-risk-assistant.git
...
This creates all CSVs in `data/`: assets (60), vulnerabilities (113), threat intel (40), business services (20), remediation hints (30).

### 3. Configure (Required — Gemini AI Key)
...
> **Note:** The Gemini API key is required. The system will not start without it.
...
Open **http://localhost:8000** — the dashboard loads and runs the analysis automatically.

> **First run note:** `sentence-transformers` will download the `all-MiniLM-L6-v2` model (~90MB) from Hugging Face. NIST SP 800-53 will be fetched from CSRC and embedded into ChromaDB. This takes ~60–90 seconds on first run; subsequent runs are instant (cached).
...
[Risk Scorer — composite score: exposure 25% · exploit 20% · CVSS 20%
        ...
        │
        ▼  Top-5 candidates
[RAG Query] → ChromaDB (NIST SP 800-53 controls, embedded locally)
        │
...
│   ├── rag_pipeline.py      # NIST embedding (sentence-transformers) + ChromaDB RAG
│   ├── llm_generator.py     # Gemini AI or template-based explanation generator
│   └── requirements.txt
├── data/
...
├── frontend/
│   ├── index.html           # Full dashboard — canvas particles, risk cards, modal
...
| `sentence-transformers` | Local embeddings (`all-MiniLM-L6-v2`, offline after first download) |
